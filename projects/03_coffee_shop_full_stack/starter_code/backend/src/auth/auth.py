...
AUTH0_DOMAIN = 'jaz-in.au.auth0.com'
ALGORITHMS = ['RS256']
API_AUDIENCE = 'drinksapi'
...
def get_token_auth_header():
    ...
    if parts[0].lower() != 'bearer':
        ...
        raise AuthError({
            'code': 'invalid_token',
            'description': 'the bearer token was invalid in format'
        }, 401)

    elif len(parts) > 2:
        raise AuthError({
            'code': 'invalid_header',
            'description':  'Authorization header must be a bearer and JWT'
        }, 401)
...
@TODO implement check_permissions(permission, payload) method
    @INPUTS
        ...
        payload !!NOTE check your RBAC settings in Auth0
...
def check_permissions(permission, payload):
    if 'permissions' not in payload:
        raise AuthError({
            'code': 'no_permissions',
            'description': 'Invalid header for permissions'
        }, 401)

    if permission not in payload['permissions']:
        raise AuthError({
            'code': 'forbidden_from_resource',
            'description': 'You are forbidden from accessing'
        }, 401)
